...
<form id="yith-wcwl-form" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ) ) ) ?>" method="post">
	...
	<div class="row">
		<div class="col-md-12" id="wl-wrapper">
			...
			<!-- WISHLIST TABLE -->
			<table class="shop_table cart wishlist_table my-lists-table" cellspacing="0" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr( $wishlist_meta['ID'] )?>">
				<thead>
				<tr>
				...
				<?php if( $is_user_logged_in ): ?>
					...
					<tr>
						<?php if ( $is_user_owner && $wishlist_meta['wishlist_privacy'] != 2 && $share_enabled ) : ?>
							<td style="padding-left: 0 !important; padding-right: 0 !important" colspan="<?php echo ( $is_user_logged_in && $is_user_owner && $show_ask_estimate_button && $count > 0 ) ? 4 : 6 ?>">
